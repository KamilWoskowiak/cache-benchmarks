...
using page_t = std::array<std::uint8_t, PAGE_SIZE>;

// TBB's concurrent_lru_cache returns handles that reference the stored value,
// so hits do not copy the page. On a miss, however, the mapping function returns
// a value which TBB assigns into its internal entry. Store the page behind a
// shared_ptr so that operation only moves/copies the small pointer object instead
// of a 16 KiB std::array.
using cache_value_t = std::shared_ptr<page_t>;
...
auto mapping(cache_key_t k) -> cache_value_t {
  ...
  // Default-initialize without zeroing first; fill() below materializes the page once.
  auto page = std::shared_ptr<page_t>(new page_t);
  const auto fill_byte = static_cast<std::uint8_t>(k + 1);
  page->fill(fill_byte);
  return page;
}
} // namespace
...
auto main(int argc, char** argv) -> int {
  ...
  // cache_size is a count of 16 KiB pages. The payload itself is heap-allocated
  // and referenced by shared_ptr to avoid TBB copying a 16 KiB value on misses.
