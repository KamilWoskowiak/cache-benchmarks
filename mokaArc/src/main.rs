...
Options:
  ...
  -s, --cache_size <N>           Cache capacity in 16 KiB pages (default: 1000)
  -k, --total_keys <N>           Total keyspace size in pages (default: 2000)
...
fn mapping(key: Key, mapping_sleep_us: u64, mapping_spin_us: u64) -> Value {
    ...
    /*
     * Materialize one real 16 KiB page.
     *
     * Moka returns values by value, so wrapping
     * the page in Arc means hits clone only the
     * Arc rather than copying the full 16 KiB.
     */
    let fill_byte = (key as u8).wrapping_add(1);

    Arc::new(vec![fill_byte; PAGE_SIZE])
}
...
fn main() {
    ...
    /*
     * Moka max_capacity is the number of entries
     * because no weigher is configured.
     *
     * Each entry now contains one Arc<Vec<u8>>
     * pointing to a 16 KiB allocation.
     */
    let cache: Cache<Key, Value> = Cache::builder().max_capacity(cfg.cache_size as u64).build();

    let payload_bytes = cfg.cache_size as u64 * PAGE_SIZE as u64;

    let payload_mib = payload_bytes as f64 / (1024.0 * 1024.0);

    eprintln!(
        "page_size_bytes={},cache_payload_mib={:.2}",
        PAGE_SIZE, payload_mib
    );
     ...
     * Prepopulate cache.
     *
     * mapping delays are disabled during warmup.
     */
    ...
    /*
     * Generate key streams outside the timed
     * benchmark region.
     */
    ...
    for thread_keys in keys {
        ...
        threads.push(thread::spawn(move || {
            ...
            for k in thread_keys {
                /*
                 * On a hit, Moka returns
                 * another Arc pointing to
                 * the existing 16 KiB page.
                 *
                 * On a miss, mapping()
                 * materializes the 16 KiB
                 * page and Moka inserts it.
                 */
                let page = cache.get_with(k, || mapping(k, mapping_sleep_us, mapping_spin_us));

                /*
                 * Prevent the optimizer from
                 * treating the returned value
                 * as completely irrelevant.
                 *
                 * We only touch one byte.
                 * Scanning all 16 KiB would
