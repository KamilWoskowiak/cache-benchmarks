...
fn mapping(key: Key, mapping_sleep_us: u64, mapping_spin_us: u64) -> Value {
    ...
    /*
     * Materialize an actual 16 KiB allocation for each cached page.
     *
     * Use a key-derived byte instead of all-zero memory so that the
     * allocator-backed pages are actually written/touched.
     */
...
fn main() {
    ...
    eprintln!("quick_cache_shards={}", cache.num_shards());
    ...
    for k in 0..(warmup_keys as Key) {
        cache
            .get_or_insert_with(&k, || {
                Ok::<Value, std::convert::Infallible>(mapping(k, 0, 0))
            })
            .unwrap();
    }
    ...
    for thread_keys in keys {
        ...
        threads.push(thread::spawn(move || {
            ...
            for k in thread_keys {
                cache
                    .get_or_insert_with(&k, || {
                        Ok::<Value, std::convert::Infallible>(mapping(
                            k,
                            mapping_sleep_us,
                            mapping_spin_us,
                        ))
